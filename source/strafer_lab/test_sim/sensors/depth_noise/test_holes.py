...
"""Tests for depth camera hole noise through the full environment pipeline.
...
Usage:
    ...
    isaaclab -p -m pytest test_sim/sensors/depth_noise/test_holes.py -v -s
"""
...
from test_sim.sensors.depth_noise.utils import (
    NUM_ENVS,
    N_SAMPLES_STEPS,
    N_SETTLE_STEPS,
    CONFIDENCE_LEVEL,
...
def test_hole_rate_on_wall_pixels(depth_env):
...
def test_hole_variance_on_wall_pixels(depth_env):
    ...
    for env_idx in range(obs.shape[1]):
        ...
        # Subsample wall pixels to prevent over-precision
        if n_wall > MAX_WALL_PIXELS_PER_ENV:
            wall_indices = torch.where(mask)[0]
            perm = torch.randperm(wall_indices.shape[0])[:MAX_WALL_PIXELS_PER_ENV]
            sub_mask = torch.zeros_like(mask)
            sub_mask[wall_indices[perm]] = True
            mask = sub_mask
            n_wall = MAX_WALL_PIXELS_PER_ENV
