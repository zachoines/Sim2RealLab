...
| [`tools-and-scripts-map`](completed/tools-and-scripts-map.md) | DGX | [#93](https://github.com/zachoines/Sim2RealLab/pull/93) | ready for review. Index context module listing the project's tools + scripts + purposes (pointers to per-package README inventories as SoT) + the revisit-consumers-on-API-change rule. Sequenced after #90/#92. Brief shipped to `completed/` in-PR; drop this row on merge. |
| [`workspace-env-name-sweep`](completed/workspace-env-name-sweep.md) | DGX | [#94](https://github.com/zachoines/Sim2RealLab/pull/94) | ready for review. Reworded the ~11 pre-existing `.venv_vlm` / `env_isaaclab3` mentions in `source/**/*.py` to requirement-based wording per `conventions.md` → "Workspace environment names". Docstring/comment-only; `rg` clean; all changed files `py_compile` clean. Brief shipped to `completed/` in-PR; drop this row on merge. |
