...
| [`nx-docker-bringup`](active/trained-policy/nx-docker-bringup.md) | Jetson | [#157](https://github.com/zachoines/Sim2RealLab/pull/157) | Containerized `strafer_ros` on the Orin NX 16GB — 2 images over CycloneDDS (`strafer-cpu` + `strafer-gpu`), sim/deploy/dev compose, hybrid coupling + fail-loud guard. **Both e2e gates PASS on-device (skip 0, ProcRoom, DGX bridge):** containerized hybrid smoke (inference **+** rolling-subgoal up, fail-loud clean; TRT cold ~84 s / warm ~4 s) and the **skip-0 mission** — trip-free steady-state (2 startup `subgoal` trips, 0 mid-mission across 242 s), obs→`cmd_vel` loop held, warm inference 102 ms. Semantic VLM→policy mission validated end-to-end too. The rev-3 runbook's Gate-2 topology was materially incomplete (only ran in-session because a stale sim container silently supplied the missing nodes); 4 gaps fixed in-branch: sim-bridge `use_sim_time` (`51e42cf`), missing SLAM support nodes → opt-in `sim-perception` service (`f34e96d`), `make submit` ROS-sourcing + `pillow` dep (`1889ce5`). Open follow-ups: policy parks near goal without tripping `NavigateToPose`'s success radius; mission-runner per-step nav timeout is wall-clock (too short under sim RTF); + deferred hardware lanes. Recommends **RETIRE** on Nano-parity. **Pending human merge/review.** |
...
| [`nx-docker-bringup`](active/trained-policy/nx-docker-bringup.md) | P1 | e2e gates PASS (#157) — skip-0 mission trip-free + hybrid smoke green; sim-bridge topology gaps fixed in-branch; pending merge/review | Jetson |
