...
(the ProcScene configuration).  These functions use ``net_forces_w`` instead
of ``force_matrix_w`` to detect collisions against arbitrary scene geometry.

Test strategy:
    1. Create a NoCam env with no obstacle filter on the contact sensor
       (mimics StraferSceneCfg_ProcScene).
    2. Verify ``net_forces_w`` shape is (N, 1, 3) and is always populated.
    3. With no obstacles nearby, verify both penalties return 0.0.
    4. Place an obstacle in front, drive into it, verify both penalties fire.
    5. Verify ``collision_sustained_penalty_net`` scales with intensity.

Usage:
    cd C:\\Worspace\\IsaacLab
    .\\isaaclab.bat -p -m pytest ..\\source\\strafer_lab\\test\\rewards\\test_collision_rewards_procscene.py -v
"""
...
def _get_or_create_env():
    ...
    This mimics the StraferSceneCfg_ProcScene contact sensor configuration:
    no ``filter_prim_paths_expr``, relying on ``net_forces_w`` for collision.
    We still keep box obstacles so we can place them for controlled tests.
    """
    global _module_env
    ...
    # Replace contact sensor with filter-free version (ProcScene style)
...
def procscene_env():
    """Provide Strafer environment configured for ProcScene collision testing."""
...
def _place_obstacle_in_front(env, obstacle_name: str, distance: float = 0.4):
    """Place an obstacle directly ahead of each robot."""
...
def _move_obstacle_far_away(env, obstacle_name: str):
    """Move an obstacle far from all robots."""
    obstacle = env.scene[obstacle_name]
    device = env.device
    num_envs = env.num_envs
    env_origins = get_env_origins(env)
...
class TestNetForcesWDiagnostics:
    ...
    def test_net_forces_w_shape(self, procscene_env):
    ...
    def test_force_matrix_w_is_none_without_filter(self, procscene_env):
...
class TestNoCollisionBaseline:
    ...
    def test_collision_penalty_net_zero_when_no_contact(self, procscene_env):
        """With obstacles far away, collision_penalty_net must return 0.0."""
        env = procscene_env
        env.reset()
    ...
    def test_collision_sustained_net_zero_when_no_contact(self, procscene_env):
        """With obstacles far away, collision_sustained_penalty_net must return 0.0."""
        env = procscene_env
        env.reset()
...
class TestCollisionDetectionNet:
    """Drive robots into obstacles and verify detection via net_forces_w."""

    def test_collision_detected_via_net_forces(self, procscene_env):
    ...
    def test_collision_penalty_net_fires_on_impact(self, procscene_env):
...
class TestSustainedVsBinaryNet:
    ...
    def test_sustained_net_nonzero_during_contact(self, procscene_env):
