...
- **Trained-policy execution** (`strafer_inference/`) — `inference_node` loads an exported policy (ONNX/TorchScript) and drives `/strafer/cmd_vel` from a `navigate_to_pose` goal: variant-aware obs assembly (NOCAM/DEPTH ±SUBGOAL), a six-source freshness watchdog, recurrent hidden-state resets at mission boundaries, and an L1 velocity clamp. A depth variant ticks on the timer, and an inferring tick consumes the newest cached depth frame whether or not it advanced (`depth_tick_semantics: timer_reuse`, the shipped default), so the recurrent state, the proprioceptive terms and the command all move at the trained cadence however slowly depth arrives; the depth watchdog (`depth_timeout_s`) is what bounds the reuse. `depth_tick_semantics: gated` is the named fallback that runs one inference per fresh depth frame instead and schedules the tick off frame *arrival* (`tick_on_depth`). The timer is the watchdog heartbeat either way; the tick period is the step period the loaded artifact's sidecar records, falling back to the shared training constant for an artifact that records none, and a disagreement between the two is logged with both values; a periodic `cadence:` line reports achieved Hz against that target, inferring ticks split fresh vs reuse, every skip by cause, and the age of each consumed depth frame from publisher stamp to inference — the instrument that says what the reuse cost — and escalates to a warning that names the owner (transport vs node) below 90% of target. Depth is decoded by its detected encoding: `16UC1`/`mono16` (the RealSense driver's Z16 millimetres) and `32FC1` (the Isaac bridge's metres); anything else counts `bad_encoding`, and a frame at any resolution other than the policy's 640×360 counts `bad_shape` and is dropped (its warning throttled to once per 5 s). Z16's "no return" is `0`, so that decode carries an explicit validity mask with the cached frame and invalid pixels enter the block median as `DEPTH_MAX` (6.0 m), the training observation term's non-finite rescue, while genuine sub-0.4 m returns keep the 0.2 m nearfield fill (a surface inside the sensor's ~0.24 m minimum range also returns `0`, so it reads 6.0 m where training shows 0.2 m); the `cadence:` line ends with the Z16 frame count and the share of policy cells a majority-invalid block forced to the far clamp over that window. The depth subscription's reliability is a per-lane lever (`depth_reliability`, overridden by `STRAFER_DEPTH_RELIABILITY`) at a fixed history depth of 1 — best-effort where the publisher's QoS is not pinned, reliable on the sim-bridge lanes, whose publisher is RELIABLE and whose recovered fragments are inferences the recurrent policy would otherwise skip. `subgoal_generator_node` follows the inference node's active-goal telemetry, replans via Nav2's `ComputePathToPose`, and rolls a subgoal along the planned path for the hybrid backend. It anchors **one** path per mission (`subgoal_anchoring: mission`, the shipped default) and advances a monotonic arc-length cursor along it, matching training's one-path-per-goal contract; a fresh plan counts as planner liveness and replaces the anchored path only under the admission rules (goal changed, the remaining path is blocked on the global costmap, or cross-track exceeds `admission_cross_track_m`). `subgoal_anchoring: rolling` is the named legacy fallback that re-roots on every plan. The action server is advertised only when a policy loads (else the autonomy dispatcher falls back to Nav2).
