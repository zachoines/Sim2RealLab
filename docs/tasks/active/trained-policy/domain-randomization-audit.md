...
- **Depth stream holds.** The depth frame fails to advance for a run of steps,
  ...
  length are independent knobs. Both tiers band the fraction over `(0.0, 0.35)`
  with runs of `(1.0, 1.6)` steps and no burst component — centred on the
  arrival rate a healthy deploy stream sustains, roughly 23 Hz against a 30 Hz
  tick, and leaving about 19 Hz at the top of the band. This is the one sensor
  axis where ROBUST does not reach past REAL: the fraction is drawn per env, so
  a ceiling near `0.60` puts part of the batch at the ~12 Hz regime where a
  converged policy loses about a third of its completions, and an episode's
  outcome there turns more on its draw than on its actions. A leg trained
  against that ceiling never annealed; the same recipe under this law reached
  the reference's converged off-path rate. The memoryless
  `frame_drop_probability` still differs by tier and composes with the hold as
  a union: a repeat is either.
...
- **Command holds.** A control step on which no new command arrives, so the
  chassis re-executes the last one. Same run-length law, `(0.0, 0.05)` on both
  tiers — deliberately far below the depth band, because this models the
  residual left after the deployed node's inference cadence rather than the
  cadence itself, and it rides the same tier equality as the depth hold since
  the two compose on one episode.
...
| Depth frame holds | `(0.0, 0.35)` hold fraction over runs of `(1.0, 1.6)` steps, no burst component, on **both** tiers. Per env, re-drawn at reset. | The deployed node's own arrival statistics: ~23 Hz against a 30 Hz tick when healthy, ~12 Hz with bursty stalls at its worst. | **Closed, ceiling set by trainability.** A ceiling reaching the ~12 Hz worst case puts part of the batch where a converged policy loses about a third of its completions, and the episode's outcome turns on its draw rather than its actions; a leg trained against it never annealed. The rig's worst case is therefore an evaluation profile, not a training band. |
| Command holds | `(0.0, 0.05)` on both tiers, same run-length law | A control step on which the node publishes nothing, so the chassis re-executes its last command. | **Closed, deliberately small.** Kept far under the depth band: this is the residual after the node's inference cadence, and sizing it like the cadence would model the same stall twice. Its correct size follows whatever inference semantics the node ends up with. |
