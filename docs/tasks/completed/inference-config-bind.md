...
`inference.launch.py` sets `parameters=[<config_file: the yaml>, {model_path, policy_variant, use_sim_time}]` — the override dict is **last**. launch_ros writes that dict to a temp params-file and appends it after the yaml, so it wins over the `/**` wildcard **by order alone**: across separate params files rcl applies last-wins per key, with **no** cross-file specificity tiebreak (the FQN-beats-`/**` rule holds only *within* a single file — an override file listed first would lose to the `/**` yaml). So env keeps ownership of `model_path` / `policy_variant` / `use_sim_time` **because the dict is last**: the stale `policy_variant: "DEPTH"` literal could never override the env-driven `DEPTH_SUBGOAL`, and `test_override_dict_is_last_in_parameters` is the sole load-bearing guard — reordering `parameters=[]` would silently flip ownership. Pinned by an in-process rcl test: `/**` binds `trt_engine_cache_path` to the namespaced node; the bare key leaves it at the empty default.
