...
Full interpreter state: `pip-freeze-env_isaaclab3.txt`,
...
Every file this record names, `env-facts.md` included, is in the evidence
deposit rather than this directory; section 10 gives the repository, the
commit, the digests, and the one command that restores the directory so the
...
Raw: `physics/pose_trace_run1.npz` + `physics/pose_trace_run1.json`, and the
`run2` pair.
...
Reproduce the tables from the record's CSVs, and diff runs pairwise:
...
Raw: `eval/cadence_20260814_160942.jsonl`
...
Preimages: `goldens/preimages/` — one JSON per hashed object,
`contract-<variant>.json`, `depthobs-RLDepth_Real.json`, `layout-depth.json`,
`layout-nocam.json`, `palette-pre_enrichment.json`. Recomputed values and the
stored constants sit side by side in `goldens/golden-hashes.json`, which also
records which observation profile each of the 22 variants resolves to.
...
are in the evidence deposit (section 10).
...
Raw: `export-anchors/export-trajectories.npz` (3.78 MB) +
`export-anchors/export-trajectories-manifest.json` (per-sequence moments,
artifact SHA256s, sidecar contents, and the torch / ORT / onnx / onnxscript /
rsl-rl / numpy versions the numbers were produced under).

### Preserved binaries

Copied out of this repository, byte-verified against the live tree, and **not
committed here** — see section 10 for where they are deposited:
...
the failing imu output. There is no XML of the failure, and the two
imu files carry a `-RERUN` suffix so the artifact says so on its own:

| file in `suites/kit-junit-xml/` | which run | contents |
|---|---|---|
...
- **The record's imu XMLs are the rerun, 4/4 pass.** Summing those XMLs
...
`suites/orphaned-standalone.txt`.
...
`render/exposure-train-video.txt`.
...
`training/training-curve.csv`. The five sampled
rows above, machine-readable:
`training/curve-iters.json`.

---

## 10. Evidence deposit

The files this record names are not in this directory. They are in the
companion evidence repository
`https://github.com/zachoines/Sim2RealLab-Artifacts`, a private repository
holding the evidence behind these records, under
`isaac-lab-upgrade-baseline-2026-08-14/record-files/`, deposited at
commit `ae3e8c0ef50c80ca7832df3dec8d1c2831a5bb12`. That directory mirrors
this one, so a path named above names the same file there. The probe and
analysis scripts resolve their inputs as
`docs/measurements/isaac-lab-upgrade-baseline-2026-08-14/…` relative to the
working directory, so restoring the deposit into that path is what makes the
commands shown throughout this record run:

```
cp -a <clone>/isaac-lab-upgrade-baseline-2026-08-14/record-files/. \
    docs/measurements/isaac-lab-upgrade-baseline-2026-08-14/
```

The deposit's own `DEPOSIT.md` comes across with the files and is not part
of the record.

This section replaces the record's former file manifest. Of that
`MANIFEST.sha256`'s 85 rows, the 78 below are its payload rows unchanged; one
digested this README, which this edit changes; one digested `provenance.md`,
which it does not; and five digested `__pycache__` bytecode that was never
tracked. The file itself is not deposited.

sha256 of every deposited file, paths relative to this directory:

```
f3c07a0d55802fa90b029e9d2be6ad6a13a2f817203523927fb7abc950bedabe  analyze_roller_csv.py
d7a1f32a2dc10dcda3b8b95f0002e933966ef077fcaae43a19ea77c5c1fd2567  depth_obs_stats.py
7357d4d2af4289e30a30e5a355139ea5869d0ef8dc9956fab30cf20f629a822d  dump_export_trajectories.py
e884f73b3e3778bd4c47ccf04e6130dd1bc056468135a09c856a7ea669881c73  dump_golden_preimages.py
8bcebfc06e1c2e2056eda0dd46bda3d47867961725fefd263eddfc3b1e2731d3  env-facts.md
701c5369a153d4c2b481354b7a8738a2801dd40a6f1734f96b64f030611ae78a  eval/cadence_20260814_160942.jsonl
55a4bad95dbf63b16595e4233a1027c58522f8aeed06c3cc3e4530a4f1a636d7  export-anchors/export-trajectories-manifest.json
269c88d0ac62907ddfc781d662c9bb36d05931a07add0744a35b7300d83d7374  export-anchors/export-trajectories.npz
c9478d63b0f9de4dfb91014a68c1c54953fc8c28718302ebe43b4c78b3b061cc  export-anchors/export-trajectories.npz.sha256
96217df3d7b71231de5b4b8d4d9ecf52ff4dca10a05ab59faea308a7a9d0c322  goldens/golden-hashes.json
042e76e716d3725cc14ecbc965b4b4a20c620fdf187443ed0e991373ae6ade93  goldens/preimages/contract-RLDepthEnriched_Real.json
ba2956c6e57ecd334f0ccd7d0ce3fbcc3037cd5b557e39568102dc4824f46aa4  goldens/preimages/contract-RLDepthEnriched_Real_PLAY.json
c792c02e631d17d5d5ec978757d6afca1387e13f15efd1139e1e71da9ac84a4b  goldens/preimages/contract-RLDepthEnriched_Robust.json
78a06ccd7cda438183d0d22916cdf15709a35f9babf70fea65866762fb359f2d  goldens/preimages/contract-RLDepthEnriched_Robust_PLAY.json
4021ff90a25769bbe3c3ce48dede28e1e35a92607eeb272d2dd8b0678ec78691  goldens/preimages/contract-RLDepthSubgoalEnriched_Real.json
37e31cd022b0616b0b70bf3bbdff8424e59f27dc21bb0820ea9f305d25c77f9c  goldens/preimages/contract-RLDepthSubgoalEnriched_Real_PLAY.json
269146b5f529ac8f859ff844af4ce2eb4637179f8cf39b870cf80c997e792ab0  goldens/preimages/contract-RLDepthSubgoalEnriched_Robust.json
82c0548a19b48bd72e2c8395c713c8bc01f447cfade19fcb460e401265ff8246  goldens/preimages/contract-RLDepthSubgoalEnriched_Robust_PLAY.json
8806d71d28e32e349e25e69758aefad7a7eac541621bf0b0b661470796f3b6bf  goldens/preimages/contract-RLDepthSubgoal_Real.json
1641a583314be79a3d1b4a4ed9bd4748f7f0407feed6b9e576d07d01d4fc1508  goldens/preimages/contract-RLDepthSubgoal_Real_PLAY.json
3509545fab19d5e3873583da61e8eb3dc08bdf1bdd27e5cbd7c0e037cf51a3cf  goldens/preimages/contract-RLDepthSubgoal_Robust.json
1ba96e92cdc4eca630a2bfd2db05db65d861b48b890a39962112757260eace9d  goldens/preimages/contract-RLDepthSubgoal_Robust_PLAY.json
58e138842330a9b5f07f8278a12cc4b907800ff178852b4dea51f53e937a936d  goldens/preimages/contract-RLDepth_Real.json
68a8ded600a7cdbcdccdd68f0b48672699c00882c31e60d31ad5ff648ce956df  goldens/preimages/contract-RLDepth_Real_PLAY.json
0575207507ac7f1ecd11718cc827b8b0f14e35855e0976da870447b39d3eaaad  goldens/preimages/contract-RLDepth_Robust.json
3845ce8204051e346f7432c0a9bbcf38152d716bc1b365d0f0f5f74b912f15a0  goldens/preimages/contract-RLDepth_Robust_PLAY.json
09685f7f2550da2110524f3cecfa9410fe3b9a0fc4ce0e942530ba9cfacf80a5  goldens/preimages/contract-RLNoCam.json
a62ed3dea2e96c2b660d8c66492afe25772b936ac0dc9372f12d07d9f3e4bab0  goldens/preimages/contract-RLNoCamSubgoal_Real.json
9f0f2c36e40480218e955a56077c4f96b0dc6e7a5ccf5e88eeb235ec5becb49d  goldens/preimages/contract-RLNoCamSubgoal_Real_PLAY.json
4c302a461bbb72220accdb20f937cd9c5ef596927e12b3f1d1b2148215fbf3f1  goldens/preimages/contract-RLNoCamSubgoal_Robust.json
99f7480d629ce76ec3f86e8acb4a714bb2dc48204dd7e11b7714c13fa8313e89  goldens/preimages/contract-RLNoCamSubgoal_Robust_PLAY.json
eecc0a2e95f85bae02dbfe3090d5824c14e5384536a2e8f6d48e8cae2d75490e  goldens/preimages/contract-RLNoCam_PLAY.json
eaa1e30057b24234d3f9077a26a836ed42e2b48204ffaad6f0817e6ffa71e4dc  goldens/preimages/depthobs-RLDepth_Real.json
6c4cd4326c0d1ad5efb0737a708c39eadc40ea39802f005841317201c02a8e36  goldens/preimages/layout-depth.json
2a7e51cd9c4e0d6e2feee951ce2494c757f5df26e51c6dcdc76cb0134e5f4170  goldens/preimages/layout-nocam.json
4226c6b5fa5cc1fcc515624c2f0ecc1067a1f05ec648a3fed210fffc2562355f  goldens/preimages/palette-pre_enrichment.json
947b5fde0428d4d2d1fb861e1edd4f4a82ec1d294a65a709ce1cc2d42da0609a  physics/pose_trace_run1.json
7af477156b829eb4d5e541081ccde9b7220e24d5c5b5e2a7632f288a17849376  physics/pose_trace_run1.npz
9d2fa6e2efb868bae1207e960316cd9ee82bc6d5a533e147e3f7652f77f5c53f  physics/pose_trace_run2.json
7af477156b829eb4d5e541081ccde9b7220e24d5c5b5e2a7632f288a17849376  physics/pose_trace_run2.npz
225a16204b3e2c99a08b69ebec601803b3b964549399fc3cae1297518e880b48  physics/roller-metrics.json
8a40c79d2a5b63deec9729350427999a9f5881a47cb9d73e31f35bb271d37a12  physics/roller_z_pgs_run1.csv
8a40c79d2a5b63deec9729350427999a9f5881a47cb9d73e31f35bb271d37a12  physics/roller_z_pgs_run2.csv
8a40c79d2a5b63deec9729350427999a9f5881a47cb9d73e31f35bb271d37a12  physics/roller_z_pgs_run3.csv
8a40c79d2a5b63deec9729350427999a9f5881a47cb9d73e31f35bb271d37a12  physics/roller_z_pgs_run4.csv
2302cdc945252e475bc2d7987f5ed0c3ee1d3bb1f5dc27d6c24dac238e7f5d3c  physics/roller_z_tgs_control.csv
2302cdc945252e475bc2d7987f5ed0c3ee1d3bb1f5dc27d6c24dac238e7f5d3c  physics/roller_z_tgs_control2.csv
b1071494428537b0431754ef12ba4cff3f9ca56fc2b363bdd8f8ca0b9a36d046  pip-freeze-env_isaaclab3.txt
376b75d6054f5bdbc3efae1a5634f191ca6853cd46c5d3e204622e7334dd5ac6  pose_trace_probe.py
df66225a2b82c141db48c10dab1a566c5c761235a104056395679e083910ac24  render/depth_obs_enriched_robust_play.json
0c54fda6c554115791be6617281d68e15fc9e6107f392f694c2f410abfb249af  render/depth_obs_subgoal_real_play.json
ddf2739d8717a96de47363f00a6176c35d4de5a725e08656f3433cdc4f6db495  render/exposure-train-video.txt
2e7f692ff79e00829073e2e83168a79663bf815519608bc9c8a96c83c4a4ca48  suites/contract-two-file.txt
e9d42da973edbf3fff485be1a663078452fb54cf4d3485c27fcca642dd6913a7  suites/env-check.txt
7aaaffc02c2910a3f86bf3588bc61155a2ea49779724151d149e019ffea2f5b7  suites/kit-junit-xml/kit-suite-actions.xml
1f94d0b8b6454e162a093f8f0cc51bbf7e7874c33e2b4fc1efed37b50d31dab5  suites/kit-junit-xml/kit-suite-camera_jitter.xml
83699b508d09d1fb4054e1a6dd332f84a9cc17f9dcc30a3e0dab142215edda37  suites/kit-junit-xml/kit-suite-commands.xml
d002efd7100f8418dc619811d81988cbe6375854ce680121330a5d385de0c281  suites/kit-junit-xml/kit-suite-curriculums.xml
6c62e7ee994b26e0aad2299a77ba420dc8e1f364f5ce4fff45b5c5676be645c8  suites/kit-junit-xml/kit-suite-depth_noise_test_frame_drops.xml
e70e47088bb58e5fbb65d07807764977b79905d3b4182abf6992276c22cf1a8a  suites/kit-junit-xml/kit-suite-depth_noise_test_gaussian.xml
ecfcff929d5f7aad5d0fd2f0b7d4801ddd60878c0aec5463393bf3505186111f  suites/kit-junit-xml/kit-suite-depth_noise_test_holes.xml
dd21b5a402e3a3731c7b4d749b87a6387afedd904b15610fc1ee83d64638844e  suites/kit-junit-xml/kit-suite-env.xml
c49f5cff7e425615d134971a7811e70eb869fd3c817de080a0a6fc765accb87d  suites/kit-junit-xml/kit-suite-events.xml
a5dc6890a9ae1ca26475f9653a12c924e9c5fad3617f807a239ca375aa07843d  suites/kit-junit-xml/kit-suite-imu_test_imu-RERUN.xml
1333006f3c669e0969240e48e92b30a8c7f202a7b80f05e2fd66a1225de71df9  suites/kit-junit-xml/kit-suite-imu_test_imu_collision-RERUN.xml
3c0c6bc60d099a59dc2c5d9dd6775fee1eeafb44fc1b702285a7ee066119e982  suites/kit-junit-xml/kit-suite-noise_models.xml
d00fdebff7530dcd7e8788c14faedf688fd9b3c0cbc7dfa3ee78341f569b93ca  suites/kit-junit-xml/kit-suite-obs_dump.xml
85ac4c05456fd840f51b95c63d14c4c7d426fbff799b895fb47d0ee218651763  suites/kit-junit-xml/kit-suite-observations.xml
8363f1f4c31bdcccb984680c84e2f37b57b9898b8d620c57991285d28225f765  suites/kit-junit-xml/kit-suite-rewards_test_collision_rewards.xml
09623896700e7dfb62920fee7aac20264826bcdaf3b754aa3ad1e18749e53957  suites/kit-junit-xml/kit-suite-rewards_test_rewards.xml
e61dc9af2ea8ea57bbfa35502ac30a273625a11271976a950a658016c9c6c9da  suites/kit-junit-xml/kit-suite-sensors.xml
a11e8e9db4a2553d2eff14b7b57c2671c7ab073c2296468cb9f2b900097c02a4  suites/kit-junit-xml/kit-suite-terminations.xml
5ee86452b6edd2882d8eca3e1b72cb17735288a7a37edc5b4bc0099515190da0  suites/orphaned-standalone.txt
0e50c20d72b010f5d0967980f1d7ddf4c508dd072218cecc92687b2bc09f72cd  suites/test-autonomy.txt
641ac0aae950b89c68aeda4ca590a82a68bb86c1ed049d570ef16c607fb2ce9b  suites/test-ros-attempt.txt
a53b6cb6be892d34fc88019f9fa67923a5dd2ac593899ddb7d8a3fa328917ad3  suites/test-vlm.txt
8f5908a436e9ac96a9f14e616a7dc1fb8e6c45a73eb3695c64c11a2596272e80  training/curve-iters.json
e64cdfe385833b06b7109799b860d53f7e6528effc00f9b5f1457ed1caa9672b  training/training-curve.csv
```

Binaries deliberately kept out of the main repository, with hashes recorded
above: the v2 source checkpoint and the other five of its training run, every
...
20-entry `SHA256SUMS.txt` covering the model artifacts and checkpoints, in a
sibling deposit directory `upgrade_baseline_artifacts/` under the same record
name in the evidence repository, deposited at commit
`dbdab91f83aa65ebced19f064fe7cd88ec212f6b`.
