...
| [`depth-subgoal-reactive-avoidance`](parked/trained-policy/depth-subgoal-reactive-avoidance.md) | [`depth-subgoal-env`](active/trained-policy/depth-subgoal-env.md) tracking checkpoint accepted **and** operator ready to spend a DEPTH-rate run on reactive avoidance | Closes the *avoidance* half of depth-subgoal-env: the depth penalty shipped **inert** in #138 because the current ProcRoom env starves it (pre-cleared straight A* paths, a 0.3 m corridor that punishes avoidance, rear-bump collisions a forward camera can't see). Re-enable the penalty (one-float flip) in a hardened env — authored on-path gate corridor + widened depth-only off-path bound + detour-economics rebalance + spawn-heading fix; moving obstacles staged after. Value confirmed (Nav2 pre-clears static geometry but is slow on dynamics + can't give a blind agent margin in doorways). |
