...
| [`procroom-depth-enrichment`](active/trained-policy/procroom-depth-enrichment.md) F3 tall furniture | DGX | [#159](https://github.com/zachoines/Sim2RealLab/pull/159) merged | Enriched-only tall furniture/columns via a `tall_object_heights` map (palette geometry + pose-z in lockstep); XY unchanged so navigation is byte-identical. Descriptor A/B (same-session, identical rooms): D4 34.6→36.4% (+1.8, no regression; D3 7.3→6.3%), still below the band → the near-column frequency lever is a follow-up. 14 vanilla goldens unchanged, 8 enriched re-frozen; Kit-free suites green. Brief stays active — enriched retrain + NX deploy-frame confirm remain open. |
| [`procroom-placement-architecture`](active/trained-policy/procroom-placement-architecture.md) | DGX | [#160](https://github.com/zachoines/Sim2RealLab/pull/160) | Design consult response opening the enrichment v2 batch: one placement-sequence + park-rank surgery serving the D4 frequency lever, the D1 rebalance, H1 compounds and F2 walls, with the vanilla-preservation argument, the NOCAM proof obligation, two new CPU-only path statistics, and a PR split (guard-net PR first, then a zero-code re-range arm, then the surgery). Design only — no production code. All six operator findings ruled 2026-07-21 and recorded in the brief (CPU-certified byte-identity + a one-off CUDA pose-hash; pre-enrichment goldens with a reconciliation assert; F2 apertures ≥ 1.2 m, which unblocks PR-3b; D4 judged by band-floor entry under paired statistics; v2 takes the D1 side keeping a spawn fairness floor; the three unknowables assigned). Brief stays active through the implementation PRs. |
| [`procroom-placement-architecture`](active/trained-policy/procroom-placement-architecture.md) PR-0 guard net | DGX | [#161](https://github.com/zachoines/Sim2RealLab/pull/161) | Frozen-output guard for the generator's default (vanilla / NOCAM) path, **zero production diff**: a 201-case seed × difficulty × batch sweep digest, the 340-op RNG sequence, the argument-defaults pin the contract hash is blind to, palette insertion order + slot-range lockstep, an `OBJECT_SIZES` immutability fixture, the widened vanilla consumer table (all open-top RL variants + the ProcRoom bridge cfg assert generator-defaults and 7/7 difficulty), and the ladder fast-out. Goldens captured from the pre-enrichment tree; the current tip reproduces all 201 cases and the draw sequence exactly (ruling F-2's reconciliation). Every obligation mutation-tested. 805 pure + env suite green. Blocking for PR-A/PR-1. |
| [`procroom-placement-architecture`](active/trained-policy/procroom-placement-architecture.md) PR-A re-range + instruments | DGX | [#162](https://github.com/zachoines/Sim2RealLab/pull/162) | Arm A (`_ENRICH_CLUTTER_WALL_BIAS` 0.5→0.3, `_ENRICH_ROBOT_SPAWN_INFLATION` 2→**1**) plus the instrument build-out. Erosion ships at 1, not 0: ruling F-5's "the floor holds by construction" is measurably false — the shared pool's structural floor is provably 0.2550 m against the 0.28 m the inflation is sized from, so erosion 0 leaves 0.108% of candidates inside it (vanilla 0.190%); one cell clears them, and four tests assert it including one driven through `reset_robot_proc_room` (the branch that delivers the pool is an unflagged `hasattr`, mutation-verified). Published noise floor at 512 rooms: D4 ±3.1 pts, D1 median ±0.067 m on room-level degrees of freedom. **Arm A moves D1** (near-band +1.36 / +3.33 / +3.63 across three legs, two intervals excluding zero) and **D4 is a measured null** (pooled +0.34 [−2.42, +3.05] over 1536 rooms/arm) — F-4's D4 gate fails on its CI conjunct only, so the surgery stays motivated. New `tools/path_statistics.py` + `scripts/measure_path_statistics.py` baselined on both corpora, threshold from the reference body (τ = 0.541 m excess) with a sensitivity sweep. 8 enriched goldens re-frozen (F-6(iii) re-verified: 8 runs on disk, none enriched); 14 vanilla goldens and every PR-0 literal untouched. 844 pure + 205/205 Kit env green. **Findings awaiting a coordinator ruling**: D4's same-configuration run-to-run spread is 4.8 pts against the 4.2-pt gap it gates on (sampling, not protocol — batch selection inside one run swings 4.96); the design's path-topology domain gap drew 71% of its scanned endpoints from outside the building, and measured on the in-room pool production uses, the enriched generator sits *inside* the corpus's room-scale range; the exact D4 ray cast puts 80.7% of D4-near cells on walls and 1.8% on tall columns; and `_gpu_bfs`'s unfree seed cell enters the spawn pool in 11.2% of enriched envs (PR-3a's seed-protection hazard, now measured). |
| [`procroom-placement-architecture`](active/trained-policy/procroom-placement-architecture.md) PR-1 surgery + D4 column knob | DGX | [#163](https://github.com/zachoines/Sim2RealLab/pull/163) | The surgery: `generate_proc_room` gains trailing `placement: PlacementCfg \| None` / `health_sink: dict \| None`; the furniture/clutter loops iterate pre-resolved slot **sequences**, the two-tier reversed ladder becomes a group-atomic **park rank**. Vanilla path byte-identical — 201-case digest sweep + 340-op draw sequence + every pin pass untouched, only the arg-defaults pin moves (additively). Enriched-gated, both vanilla-stream-neutral: a mid-room **column phase** (`column_prob=0.5`, `column_count=2`, radial law bounded off the BFS seed, `column_protected_park_rank` moves columns last within clutter — protection as bounded rank position, never exemption; its own draws are caged behind `column_prob > 0`) and a draw-free **seed correction** (`_gpu_bfs`'s unfree centre → nearest free cell; PR-3a's hazard pulled forward, enriched-only). **Gate A CUDA pose-hash** identical before/after (9×64-env, `83d390b8…`). **Gate B** 256-env NOCAM smoke: reset path ~9.3 s / full reset (~36 ms/env), unchanged across surgery. **D4 gate MET** at the scaled 2 688-room protocol (`--num_resets 42`): shipped `prob=0.5` reads D4 **45.45 ≥ 40.6** floor with paired improvement over off **+2.21 [+0.327, +4.087]** (excludes zero, half-width 1.88 ≲ 2). Probability set from a 512-room first-look (off→fully-on +5.70) as the widen-not-shift midpoint; the knob is a real D4 lever, larger than the 1.8% ray-cast *share* (that's existing cells; this is the marginal add). D1 moves toward target in the same arm. `--placement-override` added to the capture instrument for re-tuning. 8 enriched goldens re-froze (F-6(iii) re-verified, 14 runs, none enriched); 14 vanilla goldens + all PR-0 literals untouched. 929 pure + 69/69 contract green. Compounds/walls/coverage-invariant are PR-2/PR-3. |
| [`procroom-placement-architecture`](active/trained-policy/procroom-placement-architecture.md) PR-2 H1 compounds + overlap fix | DGX | [#164](https://github.com/zachoines/Sim2RealLab/pull/164) | The folded overlap fix ships enriched-only (`furniture_aabb_reject`: reject interpenetrating furniture by true rotated-AABB, killing the couch-inside-table archetype — furn–furn overlaps 1.59→0/room at −0.12 furniture/room L7 only, density held; the 0.5/0.4/0.3 thresholds lifted to named constants). The **H1 compound mechanism** ships in-tree but **catalogue-parked**: `CompoundCfg` + a menu-drawn wall-flush cluster phase (within-unit overlap kept so occupancy unions the composite footprint, budget-consuming, park-atomic, true-AABB reject extended to furn–compound/compound–compound, no new palette slot). **Instrument-first** built the footprint/top-z Wasserstein instrument vs the Infinigen corpus: the top-z gap is real (26% vs 49% above 1 m) but the **footprint-area gap is reversed** (Infinigen dominated by small elevated tabletop objects) → recorded non-goal, routed to H2/Tier-3; alignment dropped (bbox-only corpus). The 3-arm scaled descriptor leg (2 688 rooms/arm) ruled the catalogue: candidate D4 vs base **−0.037 [−1.98, +1.88]** — no movement toward the band interior → overlap fix ships alone. Overlap fix passes the density floor (D1 median +0.013, D4 44.22 in-band), step-rate **100%**, **F-6(i)** clean (overlapping kinematic bodies → no PhysX complaint), D5 +0.048 in the predicted direction (n.s.). K-fold proximity penalty recorded as a v2-retrain observation. 8 enriched goldens re-frozen (F-6(iii) re-verified); 14 vanilla goldens + PR-0 literals untouched; navigation CPU suite 188 + composition-contract Kit green. Compounds parked, F2 walls + coverage-invariant are PR-3. |
| [`procroom-placement-architecture`](active/trained-policy/procroom-placement-architecture.md) PR-3 F2 internal walls | DGX | [#166](https://github.com/zachoines/Sim2RealLab/pull/166) (merged `43ffcd6`) | **Step-1 STOP: F2 parked, no production diff.** The dispatch's "derive the gate before building the lever" ran as written and the gate fails. Heavy-corpus targets at the ruled protocol (sealed, band 1.5–4.0 m, τ from the heavy body = 0.566 m excess): turning **29.6%** [25.1, 33.1], bending 22.1%, turn-density p90 0.404, PS2 40.1%. The **shipped** enriched generator already reads 51.9% [48.8, 54.9] / 27.9% / 0.612 / 77.1% — above the target on all four, disjoint on three — and a built candidate lever (dead `max_iw` axis, enriched-gated, residual-stock partitions, apertures ≥ 1.2 m by construction, doorways protected from clutter) moves three of the four **further away** (turning 52.0–54.6%, PS2 79–80%). Robust to the three checks that could have rescued it: matching ProcRoom's 0.28 m inflation barely moves the corpus (29.9% turning), every quantile-derived τ orders the legs the same way, and in the 4–8 m band the generator (76.3%) already tops the corpus range (60–77%) while the 8–15 m band where corpus curvature actually lives is unreachable — a partition changes a path's arc, not its endpoints. Mechanism works (partitioned rooms 57.6% turning); the population does not need it. Also recorded: **PR-1's column knob is itself a path-topology lever** — switching it off drops turning 51.9→46.6%, 5.3 points, as much as the wall lever buys where it fires (the overlap fix is path-neutral); an inert lever still flips all 8 enriched goldens (`PlacementCfg` is in the `events` contract), which is why nothing ships; the shipped generator already leaves 66.8% of rooms multi-component (PR-3a's coverage invariant, now measured); seed relocation doubles under walls and the shipped guard absorbs it (`bfs_failed` 0). Guard net + 188 navigation tests green with the candidate in tree before it was reverted. Report: `SUBGOAL_EPIC/ENRICHMENT_PR3_STEP1_FINDINGS.md`. |
| [`procroom-depth-enrichment`](active/trained-policy/procroom-depth-enrichment.md) F4 camera-pose DR + ceiling probe (+ PR-3a counters) | DGX | [#167](https://github.com/zachoines/Sim2RealLab/pull/167) | **The v2 batch's closing implementation PR — three items, one enriched golden re-freeze.** **F4:** the rendered policy camera is pointed through the mount offset `randomize_d555_mount_offset` already samples (`jitter_d555_camera_prim_pose`, enriched depth cfgs only), so the sensor sim stops claiming a tolerance its images do not have. It consumes the realization — no new band, no new draw — and takes the *inverse* of the observation-side rotation (the obs resolves a body vector on the misaligned axes; the prim carries the housing rotation), pinned by a unit test. The ±1°/±3° bands moved to one shared constant per tier, hash-neutrally. Mechanism settled empirically before the code was written: a USD local-xform write on the camera prim **does** reach RTX, is per-env exact, and survives stepping. Descriptor leg (2 688 rooms/arm, paired, seed 20260101): **stream neutrality measured** — 100.0% same difficulty, 100.0% byte-identical geometry (PR-2's overlap fix read 3.3%); **D4 44.220 → 44.215** (−0.004 [−0.030, +0.022]), **D3 6.130 → 6.125** (−0.005 [−0.030, +0.020]), **D1 1.657 → 1.661** (+0.004 [+0.002, +0.006] — 4 mm, inside the recorded band). The intended row-profile softening shows: floor pixels 29.42 → 27.36% (−2.05 [−2.18, −1.92]), D5 1.850 → 1.875%. Step-rate 13.56 / 13.50 env.step/s. **Ceiling probe: closed, nothing changed.** RTX *does* honour `singleSided` on both the RGB and `distance_to_image_plane` paths (down-facing quad culled: pixel share 0.3853 → 0.1926, exactly one of two; RGB 5.1× its noise floor) — but the shipped slab is a closed cuboid whose top face still faces a camera above, so the flag is inert on it: overhead ceiling share 0.952049 → 0.952049, policy depth max|Δ| **0.0 m**. A one-way ceiling needs an open single-quad surface, a geometry and collider change, not a flag; overhead recording still needs the global-hide rig. **PR-3a counters: instrumentation only** — `free_components` / `free_multi_component_envs` / `free_largest_share_sum` / `free_largest_share_min`, inside the sink's own guard, 8-connected on the BFS's neighbourhood, validated cell-for-cell against an independent flood fill. Reproduces the finding that motivated the invariant (2.574 components/room, 66.8% multi-component, 94.25% mean largest share) and reads 2.584 / 67.5% / 93.96% on the leg's 2 688 GPU rooms; vanilla is *more* fragmented (3.377 / 79.9% / 89.00%). No enforcement — re-trigger is v2 evaluation evidence. **Golden accounting:** exactly 8 enriched hashes re-frozen, the other 14 byte-identical, hash-verified before and after each item so items 2 and 3 are *shown* golden-neutral, not assumed; F-6(iii) re-verified (8 runs on disk, none enriched). 980 pure (was 945) + 3 new Kit camera-jitter tests. **Not in this PR:** the retrain, any F2 revival, catalogue un-parking, overhead-recording tooling, and PR-3a's "promote the path statistics to durable gates" half, which stays open. |
| [`procroom-depth-enrichment`](active/trained-policy/procroom-depth-enrichment.md) F1 one-way ceiling | DGX | [#168](https://github.com/zachoines/Sim2RealLab/pull/168) | **The enclosure becomes an open down-facing quad, so an overhead `--video` camera sees the room and the robot still sees a ceiling.** One `UsdGeom.Mesh` quad with a custom `singleSided` attribute (the `doubleSided` schema attr is ignored by RTX), `/rtx/hydra/faceCulling/enabled` merged into the enriched cfgs' `RenderCfg.carb_settings`, surface offset half a slab-thickness below the entity pose so the plane the policy camera sees is exactly where the closed slab's underside was; no collider (zero thickness has no collision approximation, nothing reaches 2.15 m), mass stated outright. **Acceptance:** a `--video` training smoke on `Isaac-Strafer-Nav-RLDepth-Enriched-Real-v0` at the shipped defaults — no `p_ceil=0` override — records the enclosed room's interior where `main` records a white slab (matched frame, seed 42: mean 172.4 → 53.5). Overhead ceiling pixel share 0.9724 → 0.0363 (0.0 exactly at a 2.9 m ceiling); 62.9% of the overhead image now reads the floor. **Depth:** byte-identical for the robot's own room (1 env, same seed, max|Δ| **0.0 m**); what moves is cross-env leakage — grazing rays that used to terminate on a *neighbouring* room's 0.1 m slab rim now pin at the far clamp (37/57 600 px at 16 envs; zero flips at 40 m spacing, residual 1–4 float32 ULP). 512 paired rooms: **D3 top-11 pinned +0.077 pp [+0.050, +0.108]**, D2 top row +0.002 m, D1/D4/D5/floor-share exactly +0.000, pairing 100% byte-identical geometry, step-rate 13.74 → 13.67 env.step/s. **Illumination measured, not assumed:** culling changes primary visibility only, not light transport — the enclosed room reads overhead RGB mean 74–83 across the height band (~5% crushed), legible because a ceiling under the 2.7 m walls lets the DomeLight in over them; a training-path fill light was built and measured (crushed 31.2% → 21.8%, at the cost of a hot spot) and **not** shipped. **Goldens:** zero movement, hash-verified — the contract snapshot reads neither `cfg.scene` nor `sim.render`. The probe ships as `scripts/ceiling_sidedness_probe.py` with the renderer re-probe list in the brief. 988 pure (was 980) + Kit env 205/205 + camera_jitter 3/3. **Open for ruling:** the cross-env leakage delta — gate 1 asked for exactly zero in any pixel. |
...
| [`procroom-depth-enrichment`](active/trained-policy/procroom-depth-enrichment.md) | M | Enrich the ProcRoom generator toward Infinigen-like depth statistics — density widening + enclosure (walls/ceiling) + tall furniture, every lever gated by a descriptor instrument run on generated scenes before training spend; carries a CALIBRATION PENDING slot for the NX-session descriptor deltas and the Infinigen-finetune-only fallback trigger. Filed off the 2026-07-16 Infinigen A/B provisional domain-gap finding. |
...
| Brief | Estimate | Note |
|---|---|---|
| [`d555-invalid-pixel-statistics`](active/trained-policy/d555-invalid-pixel-statistics.md) | S | Bench measurement, rides the existing 16UC1 / IMU-QoS sitting. Decides whether the depth block reduction stays a median or switches to validity-majority + centre-2x2: the decisive statistic is whether the real D555's invalid pixels cluster at 8x8-block scale. Pre-registered decision rule in the brief. Filed off `depth-camera-vfov-parity`. |
...
| Brief | Owner | Estimate | Note |
|---|---|---|---|
| [`ros-node-clock-deadlines`](active/tooling/ros-node-clock-deadlines.md) | Jetson | M | Convert hand-rolled tick counters to node-clock deadlines (`get_clock()` already **is** sim time under `use_sim_time`). Node-clock timestamps, **not** more timers. Must classify each site sim-time vs wall-clock and leave the deliberate wall-clock ones (plan-freshness, replan cadence) alone. |
...
|---|---|---|
| [`depth-subgoal-reactive-avoidance`](parked/trained-policy/depth-subgoal-reactive-avoidance.md) | [`depth-subgoal-env`](active/trained-policy/depth-subgoal-env.md) tracking checkpoint accepted **and** operator ready to spend a DEPTH-rate run on reactive avoidance | Closes the *avoidance* half of depth-subgoal-env: the depth penalty shipped **inert** in #138 because the current ProcRoom env starves it (pre-cleared straight A* paths, a 0.3 m corridor that punishes avoidance, rear-bump collisions a forward camera can't see). Re-enable the penalty (one-float flip) in a **separate depth-only** hardened env (NOCAM_SUBGOAL untouched — shared procroom, obstacles are its secondary path-enforcement): authored on-path gate corridor + widened depth-only off-path bound + detour-economics rebalance + a *fair* spawn (clear pivot radius, heading kept random); moving obstacles + more AABB-preserving procedural primitives staged after. Value confirmed (Nav2 pre-clears static geometry but is slow on dynamics + can't give a blind agent margin in doorways). |
