...
| [`occupancy-interior-fidelity`](parked/harness/occupancy-interior-fidelity.md) | Trigger: a mission set needs a room the occupancy currently seals (seed1 closet; seed2 bathroom + closet) | The omap flood marks rooms sealed by a `meshSimplification`-healed door cutout as occupied, so they're unreachable in the connectivity graph (correct w.r.t. sim colliders, wrong w.r.t. reality). Door matching is complete; this is the occupancy fidelity fix. Filed off PR #96 (the door-footprint carve was tried + rejected — regressed seed2). |
