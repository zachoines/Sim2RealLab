...
4. **Reset trigger.** :meth:`LoadedPolicy.reset` must be called by
   ...
   - **Action-server goal accepted** (a NEW ``/navigate_to_pose``
     ...
     This covers mid-mission goal updates too: the inference node's
     action server is newest-goal-wins preemptive, so a re-grounded
     goal (VLM re-grounding) arrives as a new accepted goal and is
     its own mission boundary. Rationale: the hidden state learned
     to expect monotonic progress toward the *old* goal; carrying it
     into a re-grounded mission biases the policy.
