...
3. **Target marker** — use `isaacsim.util.debug_draw.get_debug_draw_interface()` to render a colored sphere at `(target_position_3d[0], target_position_3d[1], target_position_3d[2] + 0.3)`. Persist across ticks until `end_episode`, then clear. Debug-draw lives outside Replicator's render product capture path, so the marker is operator-only — same guarantee as the planned suggested-path overlay. **Correction (2026-09-22): that guarantee does not hold.** Both cameras render a debug-draw point in RGB, the recorded perception frames included; depth is untouched. The marker is opt-in since (`--target-marker`), and any later overlay needs its own measurement — see `measurements/debug-marker-leak-2026-09-21`.
