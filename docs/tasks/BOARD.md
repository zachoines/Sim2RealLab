...
| [`procroom-depth-enrichment`](active/trained-policy/procroom-depth-enrichment.md) | P2 | active — impl merged (#156); F3/tall furniture merged (#159 — D4 34.6→36.4%, no regression, still below band); F1/enclosure closed the dominant D3 gap ~96%, fallback does not fire; the v2 batch's placement levers are designed in [`procroom-placement-architecture`](active/trained-policy/procroom-placement-architecture.md); enriched retrain + NX deploy-frame confirm open | DGX |
