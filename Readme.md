...
- **Recurrent-policy loader for DEPTH inference** — DEPTH ONNX export ships (multi-input `(obs, h_in) → (actions, h_out)` signature, see `_OnnxDepthGRUModel`). `strafer_shared.policy_interface.load_policy()` still returns a stateless `(obs) → action` callable, so recurrent deployment needs `.reset()` exposure and hidden-state threading (`loader-recurrent-state` brief). DEPTH TorchScript export is also blocked on a DeFM `BiFPN` scriptability bug, follow-up filed. Together these gate the Jetson TRT-EP path for the DEPTH MVP.
