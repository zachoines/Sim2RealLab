...
| postprocess collider fidelity (structural `none` + door pattern) | DGX | _pending_ | Follow-up off [`scene-connectivity-validation`](completed/scene-connectivity-validation.md) (#96) review: structural collider default `meshSimplification` → `none` (exact mesh — `meshSimplification` poked into doorways) + door-pattern catches all `...DoorFactory` (GlassPanel/SPLIT_GLASS previously fell to convexHull). Runtime doorway-collision fix; does not change the connectivity graph. |
