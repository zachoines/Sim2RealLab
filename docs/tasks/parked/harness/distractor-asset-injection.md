...
1. **A source-agnostic injector tool, INVOKED by the pipeline — not welded into it.** The injector (`inject_distractors.py`) operates on **any** scene USD + its `<scene>/` dir + an asset-pool manifest, depending only on contract artifacts: the scene USD, the occupancy seam (`load_occupancy` + `occupancy_to_free_space`), `rooms[].footprint_xy` + `connectivity[].via_doorway_xy`, and the shared `path_planner`. It has **no** Infinigen coupling, so a future scene source (ProcTHOR / Habitat / a hand-built USD) reuses it unchanged per contract §g/§h.2. `prep_room_usds.generate_scenes` *calls* it via a thin `_run_inject_assets` hook (mirroring `_run_postprocess` / `_run_extract_metadata` / `_run_validate_connectivity`: `_run_subprocess`, `rc != 0 → append+continue`), gated by `--inject-assets` / `--asset-pool` (default OFF), **after** export+postprocess (so the injector authors its own convexHull colliders rather than tripping postprocess's Infinigen-prim-name regexes) and **before** `extract_scene_metadata` + `validate_scene_connectivity` + `generate_scenes_metadata`. The injector authors prims under `/World/InjectedAssets/...` so they ride the runtime `/World/Room` remount; it is idempotent (clear+re-scatter a known scope). The Infinigen-pipeline plumbing (the hook in `prep_room_usds`) is the only Infinigen-side piece; the injector itself stays on the contract side.
...
- [ ] Source-agnostic `inject_distractors.py` tool (depends only on contract artifacts + the occupancy seam — grep it for `infinigen`/`Factory`/`spawn_asset` returns zero), INVOKED by `prep_room_usds.generate_scenes` via a thin `_run_inject_assets` hook behind `--inject-assets`/`--asset-pool` (default OFF), ordered after postprocess and before metadata/connectivity/spawn-points. A future scene source can reuse the tool unchanged.
