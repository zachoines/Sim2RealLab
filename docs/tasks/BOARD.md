...
| [`single-room-couch-scene-supply`](completed/harness/single-room-couch-scene-supply.md) | P1 | PR [#128](https://github.com/zachoines/Sim2RealLab/pull/128) open — code+tests+docs complete: a parametric scene generator (`--rooms <types>` tiles an EXACT layout via `PredefinedFloorPlanSolver`, duplicates OK; `--quality {high,low}` + `--texture-res`/`--object-density`/`--geometry-detail` memory knobs; named presets retired for flags), non-clobbering `scenes_metadata.py --merge`, `SCENE_USD`/`SCENE_NAME` passthrough on `make sim-bridge`, goal-(a) mission decoupled from "couch". Tiler geometry verified end-to-end (real `solidify()` + shapely). **Operator Kit-generation gate pending before merge:** generate 1-room + 2-room scenes, confirm customData `rooms[]`=1/2 + record the goal-(a) target object. | DGX |
