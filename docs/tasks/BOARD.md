...
| [`depth-subgoal-hybrid-runtime`](active/trained-policy/depth-subgoal-hybrid-runtime.md) | Jetson | (open) | Runtime + `PolicyVariant.DEPTH_SUBGOAL` (pulled forward from `depth-subgoal-env` Phase 2) + full combo test cell + sim depth-timeout override landed. Stays active for the live acceptance (load the converged checkpoint + hybrid sim mission), gated on `depth-subgoal-env` training + the rig. |
...
|---|---|---|
| [`depth-subgoal-env`](parked/trained-policy/depth-subgoal-env.md) | [`inference-package`](completed/inference-package.md) ✅ **and** [`subgoal-env`](completed/subgoal-env.md) ✅ shipped **and** soft-prereq NOCAM_SUBGOAL deployment shows a depth-recoverable failure | Closes the 2×2 deployment matrix (direct/hybrid × NOCAM/DEPTH). Depth-aware reward shaping + DEPTH-rate training run + converged checkpoint. Lifts the NOCAM_SUBGOAL costmap-trust caveat — depth lets the policy detect late-arriving obstacles. **Phase 2 (`PolicyVariant.DEPTH_SUBGOAL`) landed early** via `depth-subgoal-hybrid-runtime`; the training env + checkpoint + reward shaping remain here. |
