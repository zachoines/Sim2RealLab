...
| [`mission-generator`](active/harness/mission-generator.md) | L | Free-text mission generator with LLM-emitted waypoints (multi-room default). Canonical mission queue source for teleop and oracle drivers. Infrastructure shipped on PR [#98](https://github.com/zachoines/Sim2RealLab/pull/98); groundable spatial language landed (no compass/egocentric leakage). Gated on a real-Qwen end-to-end pass (hallucination benchmark + per-scene start-frame-grounded rate) before it ships to completed/. |
