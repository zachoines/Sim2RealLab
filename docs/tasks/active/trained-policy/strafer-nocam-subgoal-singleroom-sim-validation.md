...
- [ ] **19-dim NOCAM_SUBGOAL obs parity ≤ 1e-5.** With a recorded sim-in-the-loop rosbag, the inference node's assembled NOCAM_SUBGOAL obs (all 19 dims) matches the gym-env (`Isaac-Strafer-Nav-RLNoCam-Subgoal-Real-v0`) obs at the same sim timestamp within float32 noise (≤ 1e-5 max abs delta). Method: stand up the bringup on the Jetson with the workstation bridge producing `/clock`, `/d555/*`, `/strafer/*` (confirm via `ros2 topic hz`); workstation-side dump the gym-side assembled NOCAM_SUBGOAL obs per step with sim timestamp (bridge mode `--obs-dump-path --obs-dump-variant NOCAM_SUBGOAL`, see the LANDED note below); Jetson-side set the node's `obs_dump_path` parameter to emit the assembled obs per tick with sim timestamp; record a ≥30 s rosbag with the robot moving (`/d555/imu/filtered`, `/strafer/joint_states`, `/strafer/odom`, `/strafer/subgoal`, `/clock`); replay and join on sim timestamp asserting the bound. The deployment-side tooling has landed under `source/strafer_ros/strafer_inference/scripts/` (`obs_parity.py` + the pure `strafer_inference.parity` library; JSONL contract in `scripts/PARITY_SCHEMA.md`).
  - **Method correction: this capture DOES need a loaded artifact.** With `model_path=""` the action server is unadvertised → no mission → no `/strafer_inference/active_goal` telemetry → the generator never calls `ComputePathToPose` → no `/strafer/subgoal`, so the subgoal-referent obs never assembles and nothing is dumped. Record with a parking artifact loaded (`strafer_nocam_subgoal_v0`; parking quality is irrelevant to parity capture) and a real mission running. Until the workstation gym dumper exists, `obs_parity.py --self-check --bag <bag>` re-assembles the reference from the bag's raw topics through the same `obs_pipeline`, pinning assembly wiring/ordering/scales on the Jetson alone. (There is no depth field for this variant, so the depth-parity check from the sibling does not apply.)
  - **Workstation gym dumper — LANDED.** Bridge mode of `run_sim_in_the_loop.py` now takes `--obs-dump-path` / `--obs-dump-variant`: per env step it evaluates the same `mdp/observations.py` terms training assembles against the live bridge scene handles (IMU accel/gyro, wheel-encoder + body velocity, and — for `DEPTH*` — the depth term against the policy camera), runs them through `assemble_observation`, and appends one `scripts/PARITY_SCHEMA.md` JSONL line stamped with the step's `/clock` sim time (`strafer_lab.bridge.obs_dump`). The referent-derived triplet and `last_action` are NaN-filled — in bridge mode the rolling subgoal is the Jetson generator's, and `last_action` is node-internal — and `strafer_inference.parity.compute_obs_parity` masks NaN dims from the bound while counting and naming them (so they are accounted for, not silently passed). The tooling is complete; the strict gym-dump join gate stays open on the rig-only step: a ≥30 s rosbag with a parking artifact + mission running, then `obs_parity.py --node-dump … --gym-dump …`.
...
3. **Parity runs:** record the rosbag with a **parking artifact loaded** and a mission running (not `model_path=""` — see the Obs-parity method correction), enable the node's `obs_dump_path`, dump gym-env obs workstation-side (or run `obs_parity.py --self-check` against the bag until the workstation dumper lands), run `obs_parity.py` + `subgoal_parity.py`, assert the 19-dim and 10 cm bounds.
