...
- `command_overlay` — draw the navigation command (goal, rolling subgoal, path) and the robot's footprint onto recorded play/train video frames. The command terms create no scene geometry, so no camera renders them.
