...
| postprocess collider fidelity (structural `none` + door pattern) | DGX | [#97](https://github.com/zachoines/Sim2RealLab/pull/97) | Follow-up off [`scene-connectivity-validation`](completed/scene-connectivity-validation.md) (#96) review: structural collider default `meshSimplification` → `none` (exact mesh — `meshSimplification` poked into doorways) + door-pattern catches all `...DoorFactory` (GlassPanel/SPLIT_GLASS previously fell to convexHull). Runtime doorway-collision fix; does not change the connectivity graph. |
| [`coverage-capture-overbright-exposure`](active/harness/coverage-capture-overbright-exposure.md) | DGX | [#117](https://github.com/zachoines/Sim2RealLab/pull/117) | Over-bright d555 perception RGB (seed6/7 87/75% clipped, 25/38 fully-white frames; seed5 bimodal). Diagnosed CPU-side: Infinigen's own baked HDR emitters (`PointLampFactory` SphereLights `normalize=True`, ~4 orders above the `--light-intensity` fill) + RTX auto-exposure OFF → linear HDR clips. Fix = enable RTX histogram auto-exposure corpus-wide via `RenderCfg.carb_settings` (re-bake-free) + `coverage_capture --render-carb` probe seam + `measure_perception_exposure.py` QA gate. Numeric acceptance OPERATOR-GATED on a production ceiling-on capture; auto-vs-fixed exposure resolved by that probe. |
...
| [`infinigen-scene-corpus`](completed/harness/infinigen-scene-corpus.md) | P1 | shipped 2026-06-24 — 5 usable `high_quality_dgx` scenes (seed1/2/5/6 + bonus seed7; ≥ 4 met); index+persistence guards (#101) + launcher `-p` fix (#104) | DGX |
| [`coverage-capture-driver`](completed/harness/coverage-capture-driver.md) | P0 | shipped 2026-06-26 ([#111](https://github.com/zachoines/Sim2RealLab/pull/111)) — `('scripted','coverage')` wired as the bulk-capture default (geometric coverage plan + trained RL subgoal-follower) + realized-mount column + scene_metadata sidecar + held-out-seed splits. Live bulk capture run is operator/GPU. | DGX |
| [`coverage-spawn-from-occupancy`](completed/harness/coverage-spawn-from-occupancy.md) | P1 | shipped 2026-06-27 ([#113](https://github.com/zachoines/Sim2RealLab/pull/113), `25d1996`) — derives the capture spawn from in-driver occupancy free-space (drops the `scenes_metadata.json` read, #112 comment 3); STOP gate refuses to mask a degenerate grid. Surfaced seed2 **+ seed6** over-occupancy → `occupancy-interior-fidelity`. | DGX |
| [`occupancy-interior-fidelity`](completed/harness/occupancy-interior-fidelity.md) | P1 | shipped 2026-06-27 ([#114](https://github.com/zachoines/Sim2RealLab/pull/114), `59a19f4`) — root-caused seed2+seed6 whole-footprint over-occupancy to a `skirtingboard_support` `convexHull` phantom slab; fixed at the Infinigen bake seam (`postprocess` case-insensitive structural trim arm) so trim gets the exact mesh, + corpus re-bake. seed2 23→72 %, seed6 21→72 % free/floor-bbox; healthy 3 unchanged. 5/5 capturable. | DGX |
| [`coverage-capture-multiscene-correctness`](completed/harness/coverage-capture-multiscene-correctness.md) | P0 | shipped 2026-06-27 ([#116](https://github.com/zachoines/Sim2RealLab/pull/116), `464728a`) — capture worked on only 1/5 scenes: every `--scene` loaded the first pooled scene's USD (`_apply_infinigen_scene_setup` hardcodes `sorted()[0]`), so the scene-frame spawn+plan landed in another floorplan. Bind geometry to the requested scene + per-scene floor; seal free-space to room footprints + in-room spawn containment; pre-build grid gate + pre-traversal scene-identity hash gate. CPU-corroborated on all 5; `--video` Kit smoke is operator-run. | DGX |
| [`coverage-capture-overbright-exposure`](active/harness/coverage-capture-overbright-exposure.md) | P0 | in flight (see In flight) — over-bright d555 perception RGB; render-layer RTX auto-exposure fix; numeric acceptance operator-gated on a production ceiling-on capture. Last blocker to a usable corpus after #116. | DGX |
...
| [`mission-generator`](completed/harness/mission-generator.md) | P2 | closed 2026-06-26 — generator + shared-planner oracle + multi-room default + geometric start-frame grounding gate shipped ([#98](https://github.com/zachoines/Sim2RealLab/pull/98)/#100/#103/#106/#108/#109); metric A (LLM-text hallucination) → parked `out-of-process-mission-text-llm`, teleop/scripted `queue` + INTEGRATION Stage 5b/5c → `harness-architecture` Tier 3 | DGX |
...
|---|---|---|
| [`hybrid-mode`](parked/trained-policy/hybrid-mode.md) | [`subgoal-env`](completed/subgoal-env.md) ✅ shipped (NOCAM_SUBGOAL checkpoint produced) + [`inference-package`](completed/inference-package.md) ✅ — hard deps now met; ready to un-park (Jetson lane) | Hybrid backend extends the inference package's runtime AND consumes the `NOCAM_SUBGOAL` checkpoint produced by the env brief |
| [`strafer-hybrid-sim-validation`](parked/trained-policy/strafer-hybrid-sim-validation.md) | [`subgoal-env`](completed/subgoal-env.md) ✅ shipped **and** [`hybrid-mode`](parked/trained-policy/hybrid-mode.md) shipped **and** sim-in-the-loop rig usable | Operator-driven sim validation for the `hybrid_nav2_strafer` backend — rosbag parity (19-dim NOCAM-fields + subgoal-pose pick), per-tick + mission-start latency, the cross-room reference mission, hybrid-specific safety (plan-freshness watchdog, costmap-trust documentation). Parallel to [`strafer-direct-sim-validation`](active/trained-policy/strafer-direct-sim-validation.md); pre-filed so hybrid-mode's runtime PR can ship with unit-testable acceptance closed, same precedent as [`inference-package`](completed/inference-package.md) → strafer-direct-sim-validation. |
| [`depth-subgoal-env`](parked/trained-policy/depth-subgoal-env.md) | [`inference-package`](completed/inference-package.md) ✅ **and** [`subgoal-env`](completed/subgoal-env.md) ✅ shipped **and** soft-prereq NOCAM_SUBGOAL deployment shows a depth-recoverable failure | Closes the 2×2 deployment matrix (direct/hybrid × NOCAM/DEPTH). `PolicyVariant.DEPTH_SUBGOAL` + depth-aware reward shaping + DEPTH-rate training run. Lifts the NOCAM_SUBGOAL costmap-trust caveat — depth lets the policy detect late-arriving obstacles. Filed pre-emptively so the design questions (architecture share with DEPTH-direct, reward composition for path-tracking vs obstacle-avoidance) stay visible. |
