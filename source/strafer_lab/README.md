...
| `scripts/collect_demos.py` | Gamepad teleop for RL demo collection — writes the HDF5 that `strafer_lab.tasks.navigation.agents.ExpertDemoBuffer` reads. Shares the family-aware reader at `strafer_lab.tools.gamepad_reader` with the harness teleop driver | Isaac Sim |
