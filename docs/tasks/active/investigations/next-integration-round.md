...
3. New follow-up briefs filed under the appropriate
   [`docs/tasks/active/<epic>/`](../) subdir for every bug surfaced
   that wasn't already in the queue.
...
- [ ] **Bug reports filed.** Any new symptoms surfaced during the
      ...
      `docs/tasks/active/<epic>/<slug>.md` briefs in the same PR, per the
...
- **Refactoring the runbook.** If the run surfaces drift in the
  ...
  separate `docs/tasks/active/tooling/integration-runbook-refresh-<NN>.md`
