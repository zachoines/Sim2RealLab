...
Policy output: [-1, 1] normalized [vx, vy, omega]
        ...
        ▼
┌─────────────────────────────────────┐
│  LAYER 2a: Command Delay            │  ← USB serial latency
│  Circular buffer, 1-3 steps         │
├─────────────────────────────────────┤
│  LAYER 2b: Slew Rate Limiting       │  ← Motor + gearbox inertia
│  Max acceleration clamp             │
├─────────────────────────────────────┤
│  LAYER 2c: Motor Dynamics Filter    │  ← Combined electrical + mechanical τ
│  First-order low-pass, τ = 50ms     │
└─────────────────────────────────────┘
        │
        ▼  velocity target (rad/s)
┌─────────────────────────────────────┐
│  LAYER 1: PhysX PD Controller       │  ← RoboClaw velocity PID
│  torque = D × (target - current)    │
│  clamped by DC motor curve          │
└─────────────────────────────────────┘
        │
        ▼  torque → PhysX rigid body dynamics
     Wheel motion
```
...
```
torque = damping × (velocity_target - velocity_current)
       = 10.0 × (ω_target - ω_current)
```

This torque is then clamped by the DC motor torque-speed curve, which models back-EMF:

```
torque_max(ω) = stall_torque × (1 - |ω| / no_load_speed)
              = 2.38 × (1 - |ω| / 32.67)
...
|--------|-------------------|-----------------|
| Controller type | D-only (damping × velocity error) | Full PID (QPPS-based) |
...
**Real-world source**: USB serial write → RoboClaw command processing → motor response. Typical round-trip: 2-5ms per command × 4 motors = 8-20ms total. At 50 Hz control rate (20ms period), this is roughly 0.5-1 step of delay.
...
max_acceleration_rad_s2 = 100.0  # rad/s² → max_delta = 100 × dt per step
```

**Real-world source**: Motor + 19.2:1 gearbox inertia limits how fast velocity can change. The reflected inertia through the gearbox (J_reflected = J_motor × ratio²) creates a natural slew rate.
...
**Tuning**: If the real acceleration is ~80-120 rad/s², the sim value is already well-matched. The `Robust` preset uses 80 rad/s² for extra conservatism.
...
**How to measure** (the τ extraction procedure):
...
4. That time is τ by definition of first-order systems
5. Repeat at multiple speeds and under load

**Sim preset values**:

| Preset | τ nominal | τ range (randomized) |
|--------|-----------|----------------------|
...
**Tuning**:
- If real τ is 40ms → within Realistic range, no change needed
- If real τ is 15ms → below Robust range, lower `motor_time_constant_range[0]` in sim
- If real τ is 120ms → above Robust range, either retrain with wider range or tune RoboClaw PID to reduce response time
- RoboClaw PID tuning directly affects the effective τ -- a well-tuned PID can bring it to 30-50ms
...
|-----------|-----------------|--------------|-----------|------|
| Accel noise density | 0.0098 | 0.015 | 0.98 mg/√Hz = 0.0096 m/s²/√Hz | m/s²/√Hz |
| Accel bias stability | 0.04 | 0.06 | 40 μg | m/s² |
| Gyro noise density | 0.00024 | 0.00036 | 0.014 °/s/√Hz = 0.000244 rad/s/√Hz | rad/s/√Hz |
| Gyro bias stability | 0.0017 | 0.0025 | 0.1 °/s | rad/s |

**Conversion from noise density to per-sample std**:
```
σ_sample = noise_density × √(sample_rate_hz)
```
At 30 Hz control rate: accel σ ≈ 0.054 m/s², gyro σ ≈ 0.0013 rad/s.
...
3. Compute:
   ...
   - **Gyro noise σ**: std of each axis
4. Compare against sim values above

**If real noise is higher**: Increase sim values. The Robust preset gives 1.5× margin.
...
|-----------|-----------------|--------------|------|
| Velocity noise σ | 0.02 | 0.05 | fraction of max velocity |
...
3. Compute velocity σ
4. Normalize: σ_normalized = σ_measured / ENCODER_VEL_MAX (3000 ticks/s)
5. Compare against sim's `velocity_noise_std`

**Typical results**: At 1000 ticks/s, expect σ ≈ 10-30 ticks/s, giving σ_normalized ≈ 0.003-0.01. This is often *lower* than the sim's 0.02, which is fine.
...
Where:
- `z` = depth in meters
- `f` = focal length in pixels (673 px at 1280×720)
- `B` = stereo baseline (0.095 m for D555)
- `σ_d` = subpixel disparity noise (0.08 px typical)

This means noise **grows quadratically with distance**:

| Distance | σ_z (Realistic) | σ_z (Robust) |
|----------|-----------------|--------------|
| 0.5 m | 0.3 mm | 0.6 mm |
| 1.0 m | 1.3 mm | 2.5 mm |
...
3. Crop a central patch (e.g., 20×20 pixels) from the 80×60 downsampled image
4. Compute σ of depth values across frames
5. Plot σ vs z² -- should be linear. The slope gives `σ_d / (f × B)`.
...
Extract τ from the step response measured in Step 1.

1. Command velocity step: 0 → 1000 ticks/s
2. Log encoder velocity at ≥100 Hz
3. Find time to reach 63.2% of target → that's τ
4. Repeat for each motor (they may differ slightly due to friction/wiring)

**Pass criteria**: τ within [30ms, 80ms] (Realistic range) or [20ms, 100ms] (Robust range).

**If τ is outside range**: Adjust `motor_time_constant_range` in `sim_real_cfg.py` and retrain, or tune RoboClaw PID to bring τ into range.
...
**Pass criteria**: Noise σ within Robust envelope (see table in Section 4).

### Step 5: Encoder Noise

1. Constant-speed test, 5 seconds per motor
2. Compute velocity σ, normalize by 3000 ticks/s

**Pass criteria**: σ_normalized < 0.05 (Robust `velocity_noise_std`).

### Step 6: End-to-End Validation (the real test)

This is where you verify that everything works together.

1. **Square test**: Command a 1m × 1m square path via `cmd_vel` sequences:
...
- [ ] Motor τ measured for all 4 motors: within [20ms, 100ms]
...
- [ ] IMU accel noise σ measured: < 0.08 m/s² (Robust threshold)
- [ ] IMU gyro noise σ measured: < 0.002 rad/s (Robust threshold)
- [ ] IMU frame orientation matches sim (x-forward, y-left, z-up in body frame)
- [ ] Encoder velocity noise σ_normalized measured: < 0.05
