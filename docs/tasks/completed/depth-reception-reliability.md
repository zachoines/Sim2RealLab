...
- [`source/strafer_ros/strafer_perception/config/d555_params.yaml`](https://github.com/zachoines/Sim2RealLab/blob/bd240ba/source/strafer_ros/strafer_perception/config/d555_params.yaml) — no `depth_qos` override → real driver keeps the wrapper image default (`SYSTEM_DEFAULT` → reliable); see the Layer 1 real-driver check for the 16UC1 encoding landmine.
