...
| [`procroom-placement-architecture`](active/trained-policy/procroom-placement-architecture.md) | P2 | active — design shipped (#160), guard net merged (#161), re-range arm + instruments in flight; the surgery (PR-1) is next and stays motivated: Arm A did not resolve D1/D4 at 512 rooms and F-4's D4 gate is unmet. Three PR-A findings await a coordinator ruling (D4's capture-protocol sensitivity, the corrected path-topology comparator, the ray-cast D4 attribution) | DGX |
