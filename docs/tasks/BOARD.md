...
| [`occupancy-interior-fidelity`](active/harness/occupancy-interior-fidelity.md) | P1 | active (on [#114](https://github.com/zachoines/Sim2RealLab/pull/114)) — root cause settled: seed2+seed6 whole-footprint over-occupancy is a `skirtingboard_support` `convexHull` phantom slab. Fix being moved from the agnostic occupancy generator to the Infinigen bake seam (`postprocess` structural trim arm) + corpus re-bake after review. | DGX |
