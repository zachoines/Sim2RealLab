...
|-------------|------------|-------------|
| `make sim-bridge` | Headless (no editor viewport) | **Daily-driver for missions.** Saves ~96 ms/loop vs `--viz kit`. Visual debugging via Jetson-side topic viewer (Foxglove or MJPEG; tracked at [`jetson-headless-viewer.md`](../jetson-headless-viewer.md)). |
| `make sim-bridge-gui` | Headed, Kit editor viewport | Visual debugging only. Editor RTX render is the single biggest cost — paid twice per loop iteration with the current double-Kit-pump architecture. |
