...
| [`scene-connectivity-validation`](completed/scene-connectivity-validation.md) | DGX | [#96](https://github.com/zachoines/Sim2RealLab/pull/96) | Shipped in the PR (stamped + moved to `completed/`): verified `connectivity[]` graph + cached occupancy seam authored into scene `customData`; seed1/seed2 re-processed (45/55 + 21/36 cross-room reachable, `multi_story=False`). Touches `path-planning-architecture.md` + `mission-generator.md` (cached-occupancy seam). Mission-generator integration smoke deferred (mission-generator unshipped). Row drops on the post-merge board tidy. |
...
| [`mission-generator`](active/harness/mission-generator.md) | L | Free-text mission generator with LLM-emitted waypoints (multi-room default). Canonical mission queue source for teleop and oracle drivers. Unblocked — `scene-connectivity-validation` shipped (#96); consumes the `connectivity[]` block + the cached-occupancy seam. |
