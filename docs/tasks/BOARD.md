...
| [`coverage-spawn-from-occupancy`](active/harness/coverage-spawn-from-occupancy.md) | P1 | active — derive the coverage capture spawn from in-driver occupancy free-space; drops the `scenes_metadata.json` dependency (#112 comment 3). Corrects the seed2 diagnosis: its occupancy grid is over-occupied (~58% blocked, ~51% free/floor-bbox), **not** mis-framed — a regenerate-occupancy blocker, not a spawn bug | DGX |
...
| [`occupancy-interior-fidelity`](parked/harness/occupancy-interior-fidelity.md) | P2 | parked (filed-on-trigger) | DGX |
...
| [`occupancy-interior-fidelity`](parked/harness/occupancy-interior-fidelity.md) | Trigger: a mission set needs a room the occupancy currently seals (seed1 closet; seed2 bathroom + closet) | The omap marks a few navigable-looking rooms' interiors occupied (0 % free) even with the door open → unreachable in the graph. Door matching is complete; PR #96 experiments ruled out the wall-collider approximation AND the flood-seed location, so the omap-classification root cause is still open. Filed off PR #96 (the door-footprint carve was tried + rejected — regressed seed2). |
