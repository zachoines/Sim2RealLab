...
| Brief | Owner | Estimate | Note |
|---|---|---|---|
| [`unify-test-targets-and-ci`](active/tooling/unify-test-targets-and-ci.md) | Either | M | **Part 1 (Makefile unification) shipped** — `make test` host-dispatch + `test-dgx`/`test-jetson` umbrellas + per-suite targets, all behind one interface. **Remaining: Part 2** — the GitHub Actions CI workflow (deferred: outward-facing, needs the repo's Actions permissions + a self-hosted DGX runner for the `lab` matrix entry). |
