...
- [ ] Tests green: `make test-harness` (the pure-Python harness suite) **and** the env-cfg construction path (env cfgs import the writer indirectly). Confirm no test still imports an old `strafer_lab.tools.X` path.
