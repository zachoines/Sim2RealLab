...
**Status:** Shipped 2026-06-30 in `b706b03` (DGX) — code + tests + docs landed: `TRUE_SINGLEROOM`/`TRUE_TWOROOM` presets via `PredefinedFloorPlanSolver` (in-repo floor-plan JSONs + `Solver.floor_plan` injection), removal of the misnamed `fast_singleroom`/`windows_baseline`, non-clobbering `generate_scenes_metadata.py --merge`, `SCENE_USD`/`SCENE_NAME` passthrough on `make sim-bridge`, and the goal-(a) mission decoupled from "couch". Floor-plan geometry verified end-to-end (real `solidify()` + shapely). **Merge precondition (operator Kit gate):** generate `true_singleroom`/`true_tworoom`, confirm customData `rooms[]` = 1 / 2, and record the chosen goal-(a) target object into `strafer-nocam-subgoal-singleroom-sim-validation.md`.
