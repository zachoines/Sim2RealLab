...
| [`bridge-spawn-from-occupancy`](completed/trained-policy/bridge-spawn-from-occupancy.md) | DGX | [#127](https://github.com/zachoines/Sim2RealLab/pull/127) | Shipping in this PR — per-loaded-scene occupancy spawn (no cross-scene union / pooled-max) + `--scene-usd` re-derivation + shared `spawn_pool_from_occupancy` core. Brief stamped + moved to `completed/` here; **remove this row at merge**. Pending operator review and the joint-with-Brief-B [`single-room-couch-scene-supply`](active/harness/single-room-couch-scene-supply.md) single-room Kit smoke (goal-(a) Prereq 4). |
