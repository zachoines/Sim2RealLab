...
#
# REMOVE WHEN a switch or a pin is proven to avoid the hang. This wrapper works
...
#
# Each attempt prints one accounting line carrying the resident size, so a stall
# is identifiable against the recorded signature rather than merely timed out.
#
...
# A killed Kit leaves shared-memory segments and named semaphores behind. Clear
# only the pairs whose owning pid is gone, so a concurrent run's segments
# survive. This is cleanup after the kill, not a theory about the cause.
sweep_dead_carb_shm() {
    local f b pid
...
for attempt in $(seq 1 "$ATTEMPTS"); do
    ...
    # exactly the stall signature below. It would be relaunched to exhaustion.
    #
