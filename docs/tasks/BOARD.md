...
| [`single-room-couch-scene-supply`](completed/harness/single-room-couch-scene-supply.md) | P1 | PR [#128](https://github.com/zachoines/Sim2RealLab/pull/128) open — code+tests+docs complete: deterministic `true_singleroom`/`true_tworoom` scenes via `PredefinedFloorPlanSolver` (in-repo floor-plan JSONs + `Solver.floor_plan` injection), removed the misnamed `fast_singleroom`/`windows_baseline`, non-clobbering `scenes_metadata.py --merge`, `SCENE_USD`/`SCENE_NAME` passthrough on `make sim-bridge`, goal-(a) mission decoupled from "couch". Floor-plan geometry verified end-to-end (real `solidify()` + shapely). **Operator Kit-generation gate pending before merge:** confirm customData `rooms[]`=1/2 + record the goal-(a) target object. | DGX |
