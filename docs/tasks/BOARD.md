...
| [`procroom-depth-enrichment`](active/trained-policy/procroom-depth-enrichment.md) | P2 | active — impl merged (#156); F3/tall-furniture in flight (PR #159 — same-session A/B: D4 34.6→36.4%, no regression, still below band → near-column frequency follow-up); F1/enclosure closed the dominant D3 gap ~96%, F2 deferred, fallback does not fire; enriched retrain + NX deploy-frame confirm open | DGX |
