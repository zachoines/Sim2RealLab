...
### In-scope for the first re-enable — MUST-haves (all depth-scoped; NOCAM_SUBGOAL stays byte-identical)
...
- **M4 — spawn-heading alignment** toward the path-start tangent in `reset_robot_proc_room` (depth-only). Removes the ~10 % blind-rear-bump collisions so the collision metric is interpretable as *forward* avoidance. (Cheap and orthogonal — could land independently. Alternative if declined: report a forward-arc-only collision counter.)
...
- **N3 — richer assets: REJECTED, redirect to Infinigen.** More complex mesh shapes break the analytic-AABB contract both the BFS occupancy rasterizer and the geometric `procroom_obstacle_proximity_penalty` depend on, swap cheap primitive colliders for expensive meshes, barely change the nearest-distance penalty signal, and duplicate the realistic-mesh Infinigen corpus that already owns the sim2real geometry gap. Use Infinigen for photoreal; keep ProcRoom primitive.
...
Term shipped inert at weight 0.0 (kept wired) in #138; re-enable = warm-start, not from-scratch, not an in-run ramp; modest ~−0.25, not −1.0; **reuse the same 4 DEPTH_SUBGOAL task IDs** (obs/action layout unchanged — the 2×2 matrix is closed; a retrain is expected); all env upgrades **depth-scoped** via the `(source, profile)` reward leaf-override or depth-only cfg subclasses so shared `CommandsCfg_ProcRoom_Subgoal` / `generate_proc_room` / `EventsCfg` are never edited in place; a corridor change moves termination + penalty together; authored gates > raw density; gates taller than the camera; moving obstacles = YIELD; roof = global-hide-during-`--video` only; assets rejected. **The observation vector must not change** — if it does, the depth-obs golden flips and the tracking checkpoint won't warm-start.
...
- **Real-robot rear sensing.** The genuine fix for blind rear collisions is a rear proximity sensor (sim + hardware) — its own brief; M4 (spawn heading) only removes the *training* artifact.
