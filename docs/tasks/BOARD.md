...
| [`infinigen-scene-corpus`](active/harness/infinigen-scene-corpus.md) | DGX | M | Grow the usable corpus to ≥ 4 `high_quality_dgx` scenes (embedded customData: objects + rooms + connectivity + occupancy) and re-embed the defective seed1. Gates the harness teleop driver's "≥ 30 episodes on ≥ 2 scenes" run — current state is **one** usable scene (seed2); seed1 is a present-but-empty silent failure, seed0 is the throughput preset. Phase-1 index/persistence guards shipped; Phase-2 scene generation is operator-gated. |
