...
| [`depth-camera-vfov-parity`](active/trained-policy/depth-camera-vfov-parity.md) | DGX | [#143](https://github.com/zachoines/Sim2RealLab/pull/143) (open) | Policy-camera VFOV parity: the `vertical_aperture` fix was DISPROVEN by the Kit probe (RTX derives VFOV from render resolution, ignores the authored aperture), so the fix is **80×45** (16:9 → 56.4°, matching the real sensor). `strafer_shared` + `strafer_lab`: `DEPTH_HEIGHT 60→45`, obs_dim 4819→3619 (derived), encoders resolution-follow, probe PASS (VFOV 56.41 vs 71.13). Brief stays active — the ~16 h retrain + live validation are the operator's; v1 deploy needs the parallel Jetson downsample follow-up (merge this PR first). |
| [`bridge-scene-memory-budget-gb10`](completed/bridge-scene-memory-budget-gb10.md) | DGX | [#146](https://github.com/zachoines/Sim2RealLab/pull/146) (open) | Ships in-PR: the single-scene default binds the LIGHTEST registered scene (by resolved `.usdc` size) instead of `sorted()[0]`, so bare `make sim-bridge` can't default to a massive scene that exhausts memory at load time; `SCENE_USD` / `--scene-usd` stay the override. Training unaffected (procroom/plane, never Infinigen). Brief moved to `completed/`; remove this row on merge. Operator gate: bare `make sim-bridge` boots without OOM + the `[scene-select]` log names the scene. |
