...
| `--target-marker` | (off) | Draw a green debug-draw marker at the active target's position. It renders into both cameras' RGB, the recorded perception frames included, so it is for watching a session rather than for one being kept |
