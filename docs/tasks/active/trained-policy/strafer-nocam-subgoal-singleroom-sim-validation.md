...
4. **A single-room scene with a single unambiguous target.** Load a genuine single-room scene via `--scene-usd` / `--scene-name` (e.g. `scene_singleroom_000_seed0` — generate with `prep_room_usds.py generate --rooms living-room --quality low --name singleroom`, per the [`single-room-couch-scene-supply`](../../completed/harness/single-room-couch-scene-supply.md) brief) that contains at least one uniquely-present, groundable object, so any spawn point is in the same room as the target. Infinigen cannot guarantee a *specific* object (e.g. a couch), so the target is whatever unique groundable label the generated scene actually carries in its customData `objects[]` — phrase the mission to match that **recorded** label. Cross-room is out of scope here.
...
- [ ] **Policy-driven single-room mission completes.** A manual mission submitted via `strafer-autonomy-cli submit "go to the <recorded single-room target object>"` (the unique groundable label recorded from the scene's customData `objects[]`) against a single-room scene completes (chassis reaches the target pose within `xy_goal_tolerance`) with the NOCAM_SUBGOAL policy doing local control — **not** a silent Nav2 fallback.
