...
| [`procroom-placement-architecture`](active/trained-policy/procroom-placement-architecture.md) PR-A re-range + instruments | DGX | [#PRA](https://github.com/zachoines/Sim2RealLab/pull/PRA) | Arm A (`_ENRICH_CLUTTER_WALL_BIAS` 0.5→0.3, `_ENRICH_ROBOT_SPAWN_INFLATION` 2→**1**) plus the instrument build-out. Erosion ships at 1, not 0: ruling F-5's "the floor holds by construction" is measurably false (the 0.3 m disc is grid-quantized, so erosion 0 leaves 0.108% of pool candidates inside the 0.28 m chassis radius — vanilla leaves 0.190%); 1 is the largest reduction that holds it, and a test now asserts it. Published noise floor at 512 rooms: D4 ±3.1 pts, D1 median ±0.067 m (two-seed replicate 0.50 pts / 0.012 m apart), on room-level rather than pixel-level degrees of freedom. Arm A's effect on D1/D4 does not resolve at 512 rooms (2 of 14 intervals exclude zero; D4's sign flips between capture protocols) — F-4's D4 gate is NOT met, so the surgery stays motivated. New `tools/path_statistics.py` + `scripts/measure_path_statistics.py` baselined on both corpora with the threshold derived from the reference body (τ = 0.402 m excess) and a sensitivity sweep. 8 enriched goldens re-frozen (F-6(iii) re-verified: 8 training runs on disk, none enriched); 14 vanilla goldens and every PR-0 literal untouched. 842 pure green. **Three findings are a STOP-and-report for the coordinator**: D4 moves 5–7 points on capture protocol alone (larger than the 4.2-point gap it gates on); the design's path-topology domain gap compared a sealed procedural room against a scanned scene the robot could leave, and measured identically the enriched generator sits *inside* the corpus's room-scale range; the exact D4 ray cast puts 80.7% of D4-near cells on walls and 1.8% on tall columns. |
