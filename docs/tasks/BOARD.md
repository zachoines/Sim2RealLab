...
| [`nav2-envelope-retirement`](completed/nav2-envelope-retirement.md) | Jetson | [#139](https://github.com/zachoines/Sim2RealLab/pull/139) | Retires `envelope_factor` + `STRAFER_NAV_VEL_SCALE` (Nav2 **and** the mirrored `strafer_inference` clamp); promotes the operator-validated sim Nav2 config to the universal baseline — both lanes byte-identical by construction, single golden pin, new [`context/nav2-config-parity.md`](context/nav2-config-parity.md) lifecycle. Supersedes [`nav2-sim-real-promotion-architecture`](completed/nav2-sim-real-promotion-architecture.md) Layer C. Rig gate **PASSED** (clean, ~2× slower legs as predicted); `make test-ros` 511 tests green; ready to merge. |
