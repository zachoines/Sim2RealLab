...
> §3's "reached its D555" reads RGB yes, depth no (measured on the 80×45 camera of v1 onward). §4's
> closed-loop comparison therefore set two depth streams without markers against each other: its
> previous-stack arm had them in RGB only. The markers still are not a cause of the pre-flip
> results, because pre-flip depth never carried them. With the markers positioned, v3's completion
> is unchanged and its steering is biased 3.7° to the left.
