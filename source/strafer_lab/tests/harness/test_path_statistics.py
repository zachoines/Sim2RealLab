...
def _open_grid(rows=60, cols=60):
    """All-free occupancy plus its (trivially inflated) free-space twin."""
    occ = np.zeros((rows, cols), dtype=np.uint8)
    return occ, np.ones((rows, cols), dtype=bool)
...
class TestSampleEndpointPairs:
    ...
    def test_too_few_points_yields_nothing(self):
        assert ps.sample_endpoint_pairs(np.zeros((1, 2)), np.random.default_rng(0),
...
class TestSummarize:
    def test_arc_band_filters_by_straight_line_distance(self):
        short = _stub_stats([[0.0, 0.0], [1.0, 0.0]], [1.0, 1.0])
        long = _stub_stats([[0.0, 0.0], [9.0, 0.0]], [1.0, 1.0])
        out = ps.summarize([short, long], arc_band=(2.0, 12.0))
