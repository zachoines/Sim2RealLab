...
| [`nav2-envelope-retirement`](completed/nav2-envelope-retirement.md) | Jetson | (open from `task/nav2-envelope-retirement`; rig-gate pending) | Retires `envelope_factor` + `STRAFER_NAV_VEL_SCALE` (Nav2 **and** the mirrored `strafer_inference` clamp); promotes the operator-validated sim Nav2 config to the universal baseline — both lanes byte-identical by construction, single golden pin, new [`context/nav2-config-parity.md`](context/nav2-config-parity.md) lifecycle. Supersedes [`nav2-sim-real-promotion-architecture`](completed/nav2-sim-real-promotion-architecture.md) Layer C. Suites green (nav 41 / inference 270 / perception 36); **merge gated on the operator re-running the e2e bridge nav2 mission at the parity cap** (expect ~2× slower legs). |
