...
degrade a measurement visibly; it makes a rule fire ~55% of the time instead of
always, and logs the suppression at a throttled WARN that reads as scene noise.
...
time, and the enriched sim-bridge lane runs at **RTF ≈ 0.11**.

## Measured, 2026-08-17 rig gate

Bridge `Isaac-Strafer-Nav-Capture-Bridge-ProcRoom-Enriched-v0`, RTF measured
0.109–0.115 over the session (`/clock` advanced 2.18 s per 20.0 s wall; the
whole-session ride-along gives 173.7 s sim over 1512.5 s wall).
...
- `/global_costmap/costmap` publishes at **0.948 Hz sim** — inter-arrival
  ...
  `costmap_timeout_s`. In wall time the same gaps are ~9.1 s, so the guard fired
  continuously and `Costmap is older than 5.0 s; skipping the collision
  admission rule` was emitted at its 10 s throttle for the whole session.
- Consequence: of the three admission rules the status line advertises
  ...
  rule is live; the next arrival is ~9.1 s wall away, so the rule is
  unavailable for roughly **45%** of evaluations. The suppression is invisible
...
- `plan is stale (older than 1.0 s); suppressing rolling-subgoal output` also
  fired: the replan period is 0.5 s sim ≈ 4.4 s wall against a 1.0 s wall guard.
