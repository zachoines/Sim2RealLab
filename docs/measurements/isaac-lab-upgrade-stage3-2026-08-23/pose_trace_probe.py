#!/usr/bin/env python3
"""Seeded replayed-action pose trace — Stage 3 copy of the 2026-08-14 baseline probe.

Identical to `../isaac-lab-upgrade-baseline-2026-08-14/pose_trace_probe.py` except
for one guarded branch in ``_np`` that unwraps patch1's ``ProxyArray`` (see that
function). The baseline probe aborts on the new pin before recording anything, so
the gate cannot be run without it. The edit is proven inert by re-running THIS file
on the old pin and reproducing the baseline's `dr_hash`, `trace_hash` and
`action_hash` exactly — recorded in the Stage 3 README. Without that proof a hash
movement on the new pin could not be told apart from an artefact of the edit.
...
def main():
    ...
    def _np(t):
        """float64 numpy from a ProxyArray, torch tensor, warp array, or numpy.
        ...
        At Isaac Lab v3.0.0-beta2.patch1 there is a fourth shape. Every
        ``Articulation.data`` field returns
        ``isaaclab.utils.warp.proxy_array.ProxyArray``, which is neither a
        ``wp.array`` nor a ``torch.Tensor`` and so reaches the ``np.asarray``
        fallback, where its forwarded ``__array__`` raises on a CUDA tensor.
        It carries the underlying tensor on ``.torch``. Unwrapping that first
        is the conversion upstream's own compatibility shim performs, and the
        branch is guarded so that a pin where ``.data`` still returns warp
        arrays or torch tensors takes exactly the path it took before —
        which is what makes this copy comparable to the pre-bump baseline.
        (``root_physx_view.get_masses()`` and friends still return real warp
        arrays at patch1, so that branch is still load-bearing.)
        """
        if not isinstance(t, (wp.array, torch.Tensor)) and hasattr(t, "torch"):
            t = t.torch
