...
- **DEPTH TorchScript export on real checkpoints** — DEPTH ONNX export ships via the recurrent `_OnnxDepthGRUModel` wrapper (multi-input `(obs, h_in) → (actions, h_out)`). DEPTH TorchScript export still fails on real checkpoints: DeFM's `BiFPN` uses `sum(generator)` which trips `torch.jit.script`. Follow-up filed off the ONNX-export work. Use `--formats onnx` for DEPTH deployment today.
- **Recurrent-policy loader contract** — `strafer_shared.policy_interface.load_policy()` returns a stateless `(obs) → action` callable; recurrent artifacts (DEPTH GRU today) need `.reset()` exposure and ONNX hidden-state threading. Tracked by [`loader-recurrent-state`](../../docs/tasks/active/trained-policy/loader-recurrent-state.md).
