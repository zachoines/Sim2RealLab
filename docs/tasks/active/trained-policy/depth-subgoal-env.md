...
Smoke test (operator Kit gate): `$ISAACLAB -p source/strafer_lab/scripts/test_strafer_env.py --env Isaac-Strafer-Nav-RLDepth-Subgoal-Real-Play-v0 --num_envs 1 --pattern circle --duration 15 --video` runs without errors; the recorded clip shows the planned path and the rolling subgoal, drawn onto the frames by the video overlay, and the robot moving through the ProcRoom depth scene. The script's flag is `--env` (not `--task`), it launches via `isaaclab.sh -p`, and it auto-enables cameras for depth envs.
