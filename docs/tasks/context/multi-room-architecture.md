...
| **v2 (measurement)** active | [`room-state-eval-harness`](../active/multi-room/room-state-eval-harness.md) | Measurement substrate (V-measure, P/R, time-to-converge, connectivity P/R) — pure eval consumer over LeRobot v3 datasets from [`harness-architecture`](../active/harness/harness-architecture.md)'s scripted × coverage capture path. Ships first | All API surface; no runtime change |
