...
| Brief | Pri | State | Owner |
|---|---|---|---|
| [`executor-simplify-align-to-goal-yaw`](active/reliability/executor-simplify-align-to-goal-yaw.md) | P3 | active | Jetson |
...
| [`executor-simplify-align-to-goal-yaw`](active/reliability/executor-simplify-align-to-goal-yaw.md) | S | Section D follow-up: retire the path-lookahead pre-rotation and `compute_path_to_pose` wrapper now that MPPI handles starting heading via its critic landscape post-CPU-relief. Drops the `STRAFER_SKIP_ALIGN_TO_GOAL_YAW` env knob too. |
